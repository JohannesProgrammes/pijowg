...
CATEGORIES = {
    ...
    "Emptied the dishwasher": "data/spülmaschine.csv",
    "Took out the glass recycling": "data/altglas.csv",
    "Cleaned the oven": "data/backofen.csv",
    "Mopped the kitchen": "data/küche.csv",
}
...
PIUS_TOKEN = st.secrets["PIUS_TOKEN"]
...
USERS = {"Johannes": f"{JOHANNES_TOKEN}", "Pius": f"{PIUS_TOKEN}"}
...
def check_login():
    ...
    if USERS.get(username) == password:
        ...
        st.error(["❌ Passwort oder Benutzername falsch!", "❌ Wrong password or username!"][lng])
...
if not st.session_state["user"]:
    ...
    st.title(["🔐 Login erforderlich", "🔐 Login required"][lng], anchor=False)

    with st.form("login_form"):
        username = st.text_input(["Benutzername", "Username"][lng], key="username")
        password = st.text_input(["Passwort", "Password"][lng], type="password", key="password")
...
def load_data(csv_path):
    ...
    if response.status_code == 200:
        ...
        return pd.DataFrame(columns=[["Datum", "Date"][lng], "Account", "Name"]), None
...
def save_data(df, sha, csv_path):
    ...
    if response.status_code in [200, 201]:
        st.success(["Antwort gespeichert! ✅", "Answer was saved! ✅"][lng])
    else:
        st.error([f"Fehler beim Speichern: {response.json()}", f"Error with saving: {response.json()}"][lng])
...
st.write([f"Hallo {user}!", f"Hello {user}!"][lng])
st.write(["Wähle deinen Namen und eine Aktivität aus:", "Choose your name and your activity"][lng])
...
if st.button(["Aktion eintragen", "Log activity"][lng]):
...
if ausräumen == 0:
    st.subheader([f"Jeder muss die Spülmaschine ausräumen, denn es ist Gleichstand", f"Both have to empty the dishwasher"][lng], anchor=False)
elif user != personen[werte.index(max(werte))]:
    st.subheader([f"Du musst noch {ausräumen} mal die Spülmaschine ausräumen", f"You have to empty the dishwasher {ausräumen} times"][lng], anchor=False)
else:
    st.subheader([f"{personen[werte.index(min(werte))]} muss noch {ausräumen} mal die Spülmaschine ausräumen", f"{personen[werte.index(min(werte))]} has to empty the dishwasher {ausräumen} times"][lng], anchor=False)
...
möglichkeiten = [["Spülmaschine ausgeräumt", "Emptied the dishwasher"][lng], ["Altglas", "Took out the glass recycling"][lng]]
kategorie = st.pills(
    ["Wähle eine Kategorie", "Choose a category"][lng],
    möglichkeiten,
...
st.subheader([f"Antworten für das Thema {kategorie}", f"Answers for the category {kategorie}"][lng], anchor=False)
st.dataframe(df)
matrix = df.to_numpy()
# print(matrix)
...
bars = alt.Chart(df).mark_bar(
    ...
    tooltip=[
        ...
        alt.Tooltip('Werte:Q', title=["Wert", "Value"][lng])
    ]
)
